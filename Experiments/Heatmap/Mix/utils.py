...
def print_stats(stats):
    for key, value in stats.items():
        print("-" * 50)
        print(f"Video: {key}")
        print(f"Label 1: {value['label1']} \t Label 2: {value['label2']}")
        for i in range(5):
...
def save_heatmap(model, heatmap, video_path1, video_path2, method, label, category, mix):
    video_name = video_path1[0].split("/")[-1][:-4] + "_" + video_path2[0].split("/")[-1][:-4]
    if not os.path.exists(f'/export/home/phuber/archive/Heatmap/mix/{mix}/{method}/{model}'):
        os.makedirs(f'/export/home/phuber/archive/Heatmap/mix/{mix}/{method}/{model}')
    np.save(f'/export/home/phuber/archive/Heatmap/mix/{mix}/{method}/{model}/{video_name}_{label}_{category}.npy', heatmap)
...
def plot_heatmaps(heatmaps, dataloader, n=3):
    for i, (video, video_path1, video_path2, label1, label2, video_org) in enumerate(dataloader):
        video_org = video_org[0]
        video_name = video_path1[0].split("/")[-1][:-4] + "_" + video_path2[0].split("/")[-1][:-4]
        heatmap_path = [heatmap for heatmap in heatmaps if video_name in heatmap][0]
        heatmap_category = "_".join(heatmap_path.split("_")[9:])[:-4]
