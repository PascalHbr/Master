...
def main(arg):
    ...
    for step, (video, video_path) in enumerate(tqdm(train_loader)):
        ...
        for i in range(0, video.shape[1], 12):  # process 10 frames at once
            ...
            for j, pred_class in enumerate(pred_classes):
                if pred_class.count('person') == 0:  # check if there is only 1 person in the frame, else discard
                    frames_inspected += 1
                else:
                    valid = False
                    break
        ...
        if frames_inspected == video.shape[1]:
            if not os.path.exists(f'labels/{arg.dataset}/{arg.model}'):
                os.makedirs(f'labels/{arg.dataset}/{arg.model}')
            with open(f'labels/{arg.dataset}/{arg.model}/train_no_person.txt', 'a+') as f:
    ...
    for step, (video, video_path) in enumerate(tqdm(test_loader)):
        ...
        if frames_inspected == video.shape[1]:
            if not os.path.exists(f'labels/{arg.dataset}/{arg.model}'):
                os.makedirs(f'labels/{arg.dataset}/{arg.model}')
            with open(f'labels/{arg.dataset}/{arg.model}/test_no_person.txt', 'a+') as f:
                f.write(f'{video_path[0]} \n')
