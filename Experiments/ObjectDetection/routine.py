...
def main(arg):
    for target in ["first", "middle", "last"]:
        ...
        if arg.inference and not arg.classification:
            ...
            wandb.init(project='Person Detection', name=f"{arg.name}_{target}", config=arg, reinit=True)
        # Set device
        device = torch.device('cuda:' + str(arg.gpu[0]) if torch.cuda.is_available() else 'cpu')

        # Setup Dataloader and model
        Dataset = get_dataset(arg.dataset)
        train_dataset = Dataset('train', arg.model, target, arg.classification, arg.blackout)
        ...
        test_dataset = Dataset('test', arg.model, target, arg.classification, arg.blackout)
                    ...
                    with torch.no_grad():
                        ...
                        if epoch % 5 == 0:
                            ...
                            if step == 64:
                                ...
                                results = make_visualizations(frames, outputs, bboxes, model_save_dir, 0, True, target)
