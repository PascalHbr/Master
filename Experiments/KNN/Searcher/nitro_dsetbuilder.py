...
from nitro.util import instantiate_from_config, parallel_data_prefetch
from nitro.data.base import PatcherDataset

SEARCHERS_BASE = {
    'iwr': '/export/compvis-nfs/group/datasets/retrieval_datasets/searchers/',
    'lmu': '/export/group/datasets/retrieval_datasets/searchers/'
}
...
def load_data(config, split='train'):
    ...
    if config.target == 'cutlit.DataModuleFromConfig':
        data.prepare_data()
        data.setup()
...
class DatasetBuilder(object):
    def __init__(self,
                 ...
                 k=10,
                 img_size=None,
                 # if 'aligned' is chosen,, the image will be devided
                 # in n_patches eventually overlapping patches
                 num_workers=None,
                 max_pool_size=2e6,
                 visualize=False,
                 save=True,
        ...
        if self.searcher_savedir is None:
            ...
            self.searcher_savedir = os.path.join(SEARCHERS_BASE[host], searcher_savedir)
    ...
    def load_embeddings(self):
        ...
        self.timestamp = '-'.join(self.saved_embeddings.rstrip('/').split('/')[-1].split('-')[:-3])
        print(f'Setting timestamp to "{self.timestamp}"')

        if not os.path.isdir(
                self.saved_embeddings) and 'compvis-nfs/user/ablattma/projects/sbgm/nitro/logs/retrieval_datasets' in self.saved_embeddings:
            print('*' * 50, "WARNING", '*' * 50)
            print(f'Directory {self.saved_embeddings} has been moved, remapping base path')
            print('*' * 110)
            reldirname = \
            self.saved_embeddings.split('compvis-nfs/user/ablattma/projects/sbgm/nitro/logs/retrieval_datasets/')[
                -1].strip('/')
            ...
            if not os.path.isdir(self.saved_embeddings):
                raise ValueError(
                    f'no database found under remapped path {self.saved_embeddings}. Please check or change config manually')
        ...
        if os.path.isfile(self.saved_embeddings):
            ...
            # TODO hack for avoiding crash due to moved files
            ...
            if len(files) == 1:
                ...
                self.data_pool = {key: np.concatenate([od[key] for od in prefetched_data], axis=1)[0] for key in
                                  self.data_pool}
    ...
    def custom_collate(self):
        ...
        if isinstance(self.patch_dset.n_patches, (list, tuple, np.ndarray)):
            def collate_fn(batch):
                ...
                elem_type = type(elem)
                try:
                    return elem_type({key: custom_coll([torch.as_tensor(d[key]) for d in batch]) if key in ['patch',
                                                                                                            'patch_coords',
                                                                                                            'img_id'] else default_collate(
                        [d[key] for d in batch]) for key in elem})
                except TypeError:
                    # The mapping type may not support `__init__(iterable)`.
                    return {key: custom_coll([d[key] for d in batch]) if key in ['patch', 'patch_coords',
                                                                                 'img_id'] else default_collate(
                        [d[key] for d in batch]) for key in elem}
    ...
    def build_data_pool(self):
        ...
        if self.saved_embeddings:
            ...
            if current_len >= self.max_pool_size:
                ...
                print(
                    f'Restarting extraction as only {current_len} of overall {self.max_pool_size} examples are in data_pool.')
        ...
        for batch in tqdm(loader, desc='Extracting feature embeddings',
    ...
    def get_nn_patches(self, batched_nns):
        ...
        for nns in batched_nns:
            ...
            for nn_id in nns:
                ...
                # crop it
                current_nn_patches.append(img[patch_coords[1]:patch_coords[3],
    ...
    def train_searcher(self, k=None,
        ...
        if self.searcher_savedir is not None and os.path.isdir(self.searcher_savedir):
        ...
        searcher = scann.scann_ops_pybind.builder(
            self.data_pool['embedding'] / np.linalg.norm(self.data_pool['embedding'], axis=1)[:, np.newaxis], k, metric)
    ...
    def build_data_pool(self):
        ...
        if self.saved_embeddings:
            ...
            if current_len >= self.max_pool_size:
                ...
                print(
                    f'Restarting extraction as only {current_len} of overall {int(self.max_pool_size)} examples are in data_pool.')
                n_examples = current_len
                start_ex = current_len // self.retriever_bs + 1
                start_loader_it = start_ex
                self.data_pool = {key: [] for key in self.data_pool}
        ...
        part = int(n_examples / self.chunk_size) + 1
        for batch in tqdm(loader, desc='Extracting feature embeddings',
                          total=(self.max_pool_size - n_examples) // self.retriever_bs):
            if 'video' in batch:
                videos = batch['video']
            else:
                if self.save_embeddings:
