...
import wandb
import torchvision
from LabelGeneration.utils import get_segmentation_mask
...
class UCF101(Dataset):
    ...
    def get_all_videos(self):
        segmentation_masks = glob.glob(f'/export/home/phuber/archive/Segmentation/ucf/{self.model}/{self.mode}/*/*.npy')
...
class Kinetics400(Dataset):
    ...
    def get_all_videos(self):
        segmentation_masks = glob.glob(f'/export/home/phuber/archive/Segmentation/kinetics/{self.model}/{self.mode}/*/*.npy')
        videos = ["/export/scratch/compvis/datasets/UCF101/videos" + mask.split(f"{self.mode}")[-1][:-4] + ".avi"
                  for mask in segmentation_masks]
        videos = list(sorted(set(videos)))
        segmentation_masks = list(sorted(set(segmentation_masks)))
    ...
    def __getitem__(self, item):
        ...
        video = self.load_video(video_path)
        segmentation_masks = np.expand_dims(np.load(self.segmentation_masks[item]), axis=3)
        category = video_path.split('/')[-2].replace(' ', '_').lower()
...
if __name__ == '__main__':
    dataset = UCF101(mode='train', model='vimpac', target='all', classification=False, blackout=None)
    ...
    for i, mask in enumerate(segmentation_mask):
        wandb.log({f"frame_{i}": wandb.Image(mask.numpy())})
