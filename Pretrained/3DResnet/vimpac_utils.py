...
class SingleAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None):
        ...
        # Pad the CLS token
        x = torch.cat([cls.reshape(batch_size, 1, 1, 1, -1) + torch.zeros(*x.shape[:-2], 1, hidden_dim, dtype=x.dtype,
                                                                          device=x.device),
...
class MultiPatternAttention(nn.Module):
    def __init__(self, d_model: int, n_head: int, patterns: str, pre_activation=False, dropout=0.0, args=None):
        ...
        self.attentions = nn.ModuleList(
            SingleAttention(d_model, d_attention // len(patterns), n_head // len(patterns), dropout=dropout,
...
class ResidualAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, pre_activation: bool = False,
        ...
        self.attention_blocks = nn.ModuleList([
            MultiPatternAttention(d_model, n_head, dropout=dropout, pre_activation=pre_activation, patterns=patterns,
                                  args=args)
...
class TransformerLayout(nn.Module):
    ...
    def _preactivation_init_weights(self):
        ...
        if dist.get_rank() == 0:
            ...
            print(
                f"\tscale the dense proj layer by {self.initializer_range} * {normalizer} (1 / sqrt of {num_residual_connections})")
        cnt = 0
        for resblock in self.transformer.resblocks:
